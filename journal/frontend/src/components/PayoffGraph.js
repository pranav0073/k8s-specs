...
// ── Payoff computation ────────────────────────────────────────────────────────
function computePayoff(spotVal, dteVal, ivVal, legs) {
  const T = Math.max(0, dteVal) / 365;
  return legs.reduce((total, leg) => {
    const K    = parseFloat(leg.strike);
    const type = leg.type;
    if (!K || !type) return total;
    const sign       = leg.side === 'B' ? 1 : -1;
    const lots       = leg.lots || 1;
    const entryPrice = leg.entry_price || 0;
    const theor      = bsmPrice(spotVal, K, T, ivVal, type);
    return total + sign * (theor - entryPrice) * lots * LOT_SIZE;
  }, 0);
}
...
function PayoffTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background:'#fff', border:'1px solid #e5e5e5', borderRadius:8, padding:'8px 12px', fontSize:13 }}>
      <div style={{ fontWeight:600, marginBottom:4 }}>NIFTY {label?.toLocaleString('en-IN')}</div>
      {payload.map(p => (
...
export default function PayoffGraph({ trade }) {
  const legs = useMemo(() => JSON.parse(trade?.legs || '[]'), [trade?.legs]);

  const [spot,    setSpot]    = useState(null);
  const [avgIv,   setAvgIv]   = useState(0.15);
  ...
  useEffect(() => {
    ...
    // Try to get NIFTY spot + IV from Kite; fall back to analysis API
    Promise.allSettled([
      axios.get(`/api/kite/quotes?trade_id=${trade.id}`),
      axios.get('/api/sessions/analysis'),
    ]).then(([kite, analysis]) => {
      let s = null, iv = null;
      if (kite.status === 'fulfilled' && kite.value.data.niftySpot) {
        s = kite.value.data.niftySpot;
        const ivs = (kite.value.data.legQuotes || []).map(l => l.quote?.iv).filter(Boolean);
        if (ivs.length) iv = ivs.reduce((a,b) => a+b,0) / ivs.length;
      }
      if (!s && analysis.status === 'fulfilled') s = analysis.value.data.currentClose;
      if (s) setSpot(s);
      if (iv) { setAvgIv(iv); setIvPct(+(iv * 100).toFixed(1)); }
      setLoading(false);
    });
  }, [trade.id, legs]);
  ...
  const breakevens = useMemo(() => {
    ...
    for (let i = 1; i < chartData.length; i++) {
      ...
      if ((a.atExpiry < 0 && b.atExpiry >= 0) || (a.atExpiry >= 0 && b.atExpiry < 0)) {
        pts.push(Math.round((a.s + b.s) / 2));
      }
    }
    return pts;
  }, [chartData]);

  if (loading) return <div className="loading">Loading payoff data…</div>;

  const fmt = n => Number(n).toLocaleString('en-IN');

  // P&L at current spot for both lines
  const sigma = ivPct / 100;
    ...
    <div style={{ padding: '0 0 24px' }}>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:24, marginBottom:20, alignItems:'flex-end' }}>
        <div style={{ flex:'1 1 220px' }}>
          ...
          <input
            type="range" min={0} max={maxDte} step={1} value={dte}
          ...
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#888', marginTop:2 }}>
            <span>Expiry</span><span>Today ({maxDte}d)</span>
          </div>
        </div>
        ...
        <div style={{ flex:'1 1 180px' }}>
          ...
          <input
            type="range" min={5} max={80} step={0.5} value={ivPct}
      ...
      <ResponsiveContainer width="100%" height={340}>
        <LineChart data={chartData} margin={{ top:10, right:20, left:10, bottom:10 }}>
          ...
          />
          <YAxis
            tickFormatter={v => `₹${v >= 0 ? '' : '-'}${fmt(Math.abs(v))}`}
            tick={{ fontSize:11 }}
            width={90}
          />
      ...
      <div style={{ fontSize:11, color:'#aaa', textAlign:'center', marginTop:8 }}>
        Theoretical P&L using Black-Scholes · Lot size 65 · Not financial advice
      </div>
    </div>
  );
}
